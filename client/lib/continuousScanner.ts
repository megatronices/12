...
class ContinuousScanner {
  ...
  async start(): Promise<void> {
    ...
    console.log(
      `🔥 Continuous scanner active: ${this.SCAN_INTERVALS.length} scan loops + endpoint rotation + high-frequency scans`,
    );
  }
  ...
  private async performScan(scanIndex: number): Promise<void> {
    ...
    try {
      ...
      console.log(
        `🔍 Scan #${this.scanCounter} (Loop ${scanIndex}): ${workerStats.busyWorkers}/${workerStats.totalWorkers} workers busy`,
      );
      ...
      switch (scanType) {
        ...
        case 2:
          ...
          const tokenData = await workerPool.execute("FETCH_TOKENS", {
            params: { limit: "500" },
          });
      ...
      if (error.message && error.message.includes("429")) {
        ...
        console.warn(
          `⚠️ Rate limit hit #${this.rateLimitHits} - using fallback data`,
        );
        // Use fallback data
        tokens = getFallbackTokens();
        this.lastResults.set(`scan-${scanIndex}`, tokens);
        ...
        console.warn(
          `⚠️ Scan #${this.scanCounter} (Loop ${scanIndex}) failed:`,
          error,
        );
      }
    }
  ...
  private startHighFrequencyScans(): void {
    ...
    const batchInterval = setInterval(async () => {
      ...
      try {
        ...
        const batchData =
          await workerPool.fetchMultipleEndpoints(batchEndpoints);
  ...
  private async processNotifications(tokens: TokenPair[]): Promise<void> {
    try {
      ...
      const newNotifications =
        await notificationService.checkAndNotifyNewSignals(tokens);
      ...
      if (newNotifications > 0 || telegramAlerts > 0) {
        console.log(
          `📢 Sent ${newNotifications} desktop + ${telegramAlerts} Telegram alerts`,
        );
      }
    } catch (error) {
    ...
    const uniqueTokens = allTokens.reduce(
      (acc: TokenPair[], current: TokenPair) => {
        const isDuplicate = acc.some(
          (token) => token.pairAddress === current.pairAddress,
        );
        if (!isDuplicate) {
          acc.push(current);
        }
